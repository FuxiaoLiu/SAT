...
class TCP(nn.Module):
	...
	def __init__(self, config):
		...
		self.init_mem = nn.Linear(232, 128)
		self.init_att = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=128, nhead=4), num_layers=3)
		self.init_wp = nn.Linear(256+256, 128)
		self.wp_att = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=128, nhead=4), num_layers=3)
	...
	def forward(self, img, state, target_point):
		...
		mem = cnn_feature.reshape(cnn_feature.shape[0], cnn_feature.shape[1], -1).permute(1,0,2)
		mem = self.init_mem(mem)
		feature_emb = self.init_att(measurement_feature.unsqueeze(0), mem).squeeze(0)
		...
		for _ in range(self.config.pred_len):
			...
			new_h = self.init_wp(torch.cat([h, traj_hidden_state[:, _]], 1))
			new_feature_emb = self.wp_att(new_h.unsqueeze(0), mem).squeeze(0)
